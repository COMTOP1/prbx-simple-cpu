...
class CPUBlockDiagram:
    ...
    def draw_block(self, name, label, x, y, initial_value="", w=200, h=75,
        ...
        label_id = self.canvas.create_text(x + w / 2, y + h / 2, text=text, font=("Arial", 18), fill=font_colour, justify=tk.CENTER)
    ...
    def update_block_value(self, values: list[tuple[str, str]]):
        ...
        while i < len(values):
            name, value = values[i]
            self.canvas.itemconfigure(self.component_labels[name], text=f"{self.component_labels_initial[name]}({value})")
            i += 1